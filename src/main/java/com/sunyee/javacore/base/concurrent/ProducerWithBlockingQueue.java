 ...
 *
 * thingking:
 * 其实这个BlockingQueue比较难用代码来演示，因为put()与take()方法无法与输出语句保证同步，
 * 当然你可以自己去实现 BlockingQueue（BlockingQueue是用await()/signal() 实现的)。所以在输出结果上你会发现不匹配。
 * 例如：当缓冲区已满，生产者在put()操作时，put()内部调用了await()方法，放弃了线程的执行，然后消费者线程执行，
 ...
 */
public class ProducerWithBlockingQueue {
    private final BlockingQueue<Integer> bucket = new LinkedBlockingDeque<>(10);
    ...
    public static void main(String[] args) {
        ProducerWithBlockingQueue bucket = new ProducerWithBlockingQueue();
        for(int i = 0; i < 4; i++){
